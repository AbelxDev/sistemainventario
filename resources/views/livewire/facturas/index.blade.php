<div class="mt-3">
    ...
    <x-adminlte-card title="Gestión de Facturas" theme="info" icon="fas fa-file-invoice">
        <div class="row mb-3">
            {{-- BUSCADOR --}}
            <div class="col-md-9">
                <x-adminlte-input name="search" wire:model.live="search" placeholder="Buscar por número o proveedor...">
            ...
            <div class="col-md-3 text-right">
                <x-adminlte-button label="Nueva Factura" theme="primary" icon="fas fa-plus"
                ...
                <tbody>
                    @forelse ($facturas as $f)
                        <tr>
                            ...
                            <td>
                                <span
                                    class="badge badge-{{ $f->estado === 'pendiente' ? 'warning' : ($f->estado === 'procesada' ? 'success' : 'danger') }}">
                                    {{ ucfirst($f->estado) }}
                                </span>
                            </td>
                            <td>
                            ...
                            <td class="text-center">
                                ...
                                <div x-data="{ tooltip: false }" class="d-inline-block position-relative mx-1">
                                    <button class="btn btn-warning btn-sm"
                                        wire:click="openEditModal({{ $f->id }})" @mouseenter="tooltip = true"
                                ...
                                <div x-data="{ tooltip: false }" class="d-inline-block position-relative mx-1">
                                    <button class="btn btn-danger btn-sm"
                                        wire:click="confirmarEliminar({{ $f->id }})" @mouseenter="tooltip = true"
                                        @mouseleave="tooltip = false">
        ...
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    {{-- PDF --}}
                    <div class="mt-4" x-data="pdfUploader" @reset-pdf.window="reset()" x-cloak>
                        <label class="font-weight-bold mb-2">Archivo PDF</label>

                        {{-- Zona de arrastrar / seleccionar --}}
                        <div style="cursor: pointer"; class="border rounded p-4 text-center bg-light "
                            :class="{
                                'border-primary shadow': fileName !== '',
                                'border-danger': isDragging
                            }"
                            @click="openFileDialog()" @dragover.prevent="isDragging = true"
                            ...
                            <input type="file" accept="application/pdf" wire:model="pdf_file" class="d-none"
                                x-ref="pdfInput" @change="updateFileName($event)">
                        </div>
                        {{-- Mostrar errores --}}
                        @error('pdf_file')
                            <span class="text-danger">{{ $message }}</span>
                        ...
                        {{-- Vista previa PDF --}}
                        @if ($pdf_preview_path)
                            <iframe src="{{ asset('storage/' . $pdf_preview_path) }}" class="mt-3"
    ...
    document.addEventListener('alpine:init', () => {
        Alpine.data('pdfUploader', () => ({
            ...
            handleDrop(event) {
                ...
                this.$refs.pdfInput.dispatchEvent(new Event('change', {
                    bubbles: true
                }));
